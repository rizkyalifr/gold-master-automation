...
def generate_market_update_report(df, kurs, ai_params):
    ...
    # Hitung ulang variabel penting
    fibo = calculate_fibonacci_levels(df, ai_params['FIBO_ANCHORS'])
    poc = get_poc(df)
    
    # Hitung Score lagi untuk "Confidence"
    final_score, _, _, _ = calculate_quant_score(last_d, prev_d, poc, fibo['GOLDEN (0.618)'], ai_params)
    
    # Tentukan Status Tren & Volatilitas untuk Narasi
    ema_dist = ((price - last_d['EMA200']) / last_d['EMA200']) * 100
    regime = "Trending Bullish 🐎" if ema_dist > 5 else ("Trending Bearish 🐻" if ema_dist < -5 else "Sideways/Ranging 🦀")
    
    is_high_vol = last_d['BB_Width'] > 0.15
    vol_desc = "Volatilitas Tinggi (Gerakan kencang)" if is_high_vol else "Volatilitas Normal (Stabil)"
    
    # Tentukan Arah Narasi berdasarkan Score
    peluang_txt = "Peluang Naik" if final_score >= 50 else "Peluang Turun/Wait"
    icon_conf = "✅" if final_score >= 50 else "⚠️"
    
    now = datetime.now(pytz.timezone('Asia/Jakarta'))
    
    # Format Pesan
    msg = f"""Update Kondisi Market PAXG ({now.strftime('%d %b %Y')})
🕒 {now.strftime('%H:%M')} WIB

Posisi: {fmt_usd(price)}
Confidence: {final_score:.1f}% ({peluang_txt}) {icon_conf}
Tren: {regime}
Status: {vol_desc}
...
def run_bot():
    ...
    # 3. Process Data
    # PENTING: Variabel processed_d harus dibuat di sini agar bisa dipakai fungsi report
    ...
    # Report 2: Narasi Market Update (Pesan Kedua)
    market_update_msg = generate_market_update_report(processed_d.tail(180), kurs, ai_params)
    
    # Print ke console untuk debugging log
    print("\n" + sop_report)
    print("\n" + market_update_msg)
    ...
    if TOKEN and CHAT_ID:
        ...
        # Jeda 3 detik agar urutan pesan terjamin rapi di Telegram
        time.sleep(3)
        
        # Kirim Pesan 2
        send_telegram_alert(TOKEN, CHAT_ID, market_update_msg)
    else:
